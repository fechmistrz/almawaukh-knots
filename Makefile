...
# make's built-in implicit rules (for C, Fortran, etc.) apply to this
# project, so drop the default suffix list rather than let make search it.
.SUFFIXES:
...
src/90-appendix/table_invariants.tex: tools/convert_knotinfo_json_to_table.py tools/knotinfo_parsed.json
	{ echo; python3 tools/convert_knotinfo_json_to_table.py all tools/knotinfo_parsed.json; } > $@
