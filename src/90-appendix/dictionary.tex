...
\item \textbf{ciało z rączkami}: handlebody
...
\item \textbf{homeomorfizm wierny}: faithful homeomorphism
...
\item \textbf{izotopia fizyczna}: physical isotopy
...
\item \textbf{liczba pereł}: pearl number
\item \textbf{liczba warkoczowa}: braid number
\item \textbf{logarytmicznie wklęsły}: log-concave
...
\item \textbf{para gordyjska}: Gordian pair
...
\item \textbf{ranga południkowa}: meridional rank
...
\item \textbf{równoboczna liczba patykowa}: equilateral stick number
\item \textbf{skrzyżowanie}: crossing
(\emph{znak}: sign)
\item \textbf{spin}: writhe
\item \textbf{splot}: link
(\emph{rozszczepialny}: splittable, \emph{specjalnie alternujący}: special alternating, \emph{sznurkowy}: string)
...
(\emph{adekwatny}: adequate, \emph{alternujący}: alternating, \emph{drzewiasty}: arborescent, \emph{dziki}: wild, \emph{długi}: long, \emph{homotopijnie taśmowy}: homotopy-ribbon, \emph{ilorazowy}: quotient, \emph{lustro/lustrzany}: mirror, \emph{obramowany}: framed, \emph{odwracalny}: reversible, \emph{okresowy}: periodic, \emph{osobliwy}: singular, \emph{pierwszy}: prime, \emph{plastrowy}: slice, \emph{poskromiony/swojski}: tame, \emph{preclowy}: pretzel, \emph{rewers/odwrotny}: reverse, \emph{skręcony}: twist, \emph{skrętny/chiralny}: chiral, \emph{uchwytowo taśmowy}: handle-ribbon, \emph{wirtualny}: virtual, \emph{zespawany}: welded, \emph{zorientowany}: oriented, \emph{zwierciadlany}: achiral/amphicheiral, \emph{złożony}: composite)
\item \textbf{węzeł babski}: granny knot
\item \textbf{węzeł dokerski}: stevedore knot
\item \textbf{zgodność taśmowa}: ribbon concordance
