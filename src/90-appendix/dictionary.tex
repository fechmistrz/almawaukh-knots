...
(\emph{adekwatny}: adequate, \emph{alternujący}: alternating, \emph{drzewiasty}: arborescent, \emph{dziki}: wild, \emph{długi}: long, \emph{ilorazowy}: quotient, \emph{lustro/lustrzany}: mirror, \emph{obramowany}: framed, \emph{odwracalny}: reversible, \emph{okresowy}: periodic, \emph{osobliwy}: singular, \emph{pierwszy}: prime, \emph{plastrowy}: slice, \emph{poskromiony/swojski}: tame, \emph{preclowy}: pretzel, \emph{rewers/odwrotny}: reverse, \emph{skręcony}: twist, \emph{skrętny/chiralny}: chiral, \emph{wirtualny}: virtual, \emph{zespawany}: welded, \emph{zorientowany}: oriented, \emph{zwierciadlany}: achiral/amphicheiral, \emph{złożony}: composite)
