...
Jak długość sznurowa zachowuje się względem sumy spójnej?
Najkrótszy splot Hopfa składa się z~dwóch okręgów o~jednakowym promieniu, leżących w~prostopadłych płaszczyznach, z~których każdy przechodzi przez środek drugiego; przy grubości $1$ promień wynosi $2$, więc $\ropelength(\text{splot Hopfa}) = 8 \pi$.
Najkrótsza suma dwóch splotów Hopfa, czyli łańcuch trzech okręgów, składa się z~dwóch okręgów leżących w~jednej płaszczyźnie oraz ,,stadionu'' (dwóch odcinków połączonych półokręgami) w~płaszczyźnie prostopadłej, przy czym każdy z~okręgów przechodzi przez środek jednego z~półokręgów.
Jej długość sznurowa wynosi $8 \pi + (4 \pi + 4) = 12 \pi + 4$, czyli o~$4 \pi - 4$ mniej niż suma długości składników \cite[problem 1.74]{baykur2026}.
Katritch, Olson, Pieranski, Dubochet, Stasiak \cite{katritch1997} przypuszczali, że przy każdej sumie spójnej oszczędzamy przynajmniej tyle sznura:
\index[persons]{Katritch, Vsevolod}%
\index[persons]{Pieranski, Piotr}%
\index[persons]{Stasiak, Andrzej}%

\begin{conjecture}
    Niech $K_1, K_2$ będą węzłami albo splotami.
    Wtedy $\ropelength(K_1 \shrap K_2) \le \ropelength K_1 + \ropelength K_2 - (4 \pi - 4)$.
\end{conjecture}

Słabszą wersję dostajemy zastępując $4 \pi - 4$ pewną stałą dodatnią niezależną od typu węzła.
Nie znamy kształtu najkrótszych realizacji poza przypadkami splotów, których składowe są płaskie; w~pozostałych mamy tylko wyniki numeryczne.
...
% DICTIONARY;physical isotopy;izotopia fizyczna;-
% DICTIONARY;Gordian pair;para gordyjska;-
Można też zapytać, czym różni się izotopia zwykła od fizycznej.
Izotopia fizyczna zachowuje długość każdej składowej splotu i~wymaga, by grubość przez cały czas wynosiła co najmniej $1$.
Dwie konfiguracje tego samego splotu, które są izotopijne, ale nie są fizycznie izotopijne, nazywamy parą gordyjską.
\index{para gordyjska}%
Takie pary istnieją dla splotów o~dwóch lub więcej ogniwach (Coward, Hass \cite{coward2015}; Kusner, Kusner \cite{kusner2023}), a~ostatnio także dla niesplotów (Ayala, Hass \cite{ayala2025}), oraz dla węzłów wyższych wymiarów (Nabutovsky \cite{nabutovsky1995}; Lishak, Nabutovsky \cite{lishak2021} podają jawną konstrukcję gordyjskiego niewęzła w~$S^4$).
\index[persons]{Coward, Alexander}%
\index[persons]{Hass, Joel}%
\index[persons]{Nabutovsky, Alexander}%
Nie wiadomo natomiast, czy para gordyjska istnieje dla węzłów w~$\R^3$, w~szczególności czy istnieje gordyjski niewęzeł \cite[problem 1.71]{baykur2026}.
