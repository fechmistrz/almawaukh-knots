...
\subsection{Trychotomia Thurstona}
...
\subsection{Kryteria hiperboliczności}
Węzły hiperboliczne stanowią najliczniejszą, ale też najmniej zrozumianą rodzinę węzłów.
Sam Nead, użytkownik portalu MathOverflow napisał, że maszyneria JSJ\footnote{rozkład Jaco-Shalena-Johannsona} oraz prace innych osób pozwalają wysłowić algebraicznie kryterium Thurstona.
\index{rozkład Jaco-Shalena-Johannsona}%
Podajemy trzy równoważne postaci, od najbardziej konkretnej do w~pełni algebraicznej.

\begin{proposition}
    Niech $L$ będzie takim splotem, który nie rozszczepia się, nie jest niewęzłem, nie posiada wśród ogniw niezakłóconego węzła satelitarnego (\emph{,,undisturbed satellite knot''}) oraz nie jest węzłem torusowym.
...
\subsection{Przykłady rodzin węzłów hiperbolicznych}
...
\subsection{Ile jest węzłów hiperbolicznych?}
...
Ta obserwacja o granicy równej $1$ jest jednak tylko przypuszczeniem, i~to przypuszczeniem, które -- jak zobaczymy -- trudno nawet precyzyjnie wysłowić.
W pracy \cite{malyutin2016} Malutin pokazał, że jest ono sprzeczne z~wieloma innymi starymi hipotezami teorii węzłów:~\ref{con:malyutin1} --~\ref{con:malyutin4}.
...
\subsection{Objętość hiperboliczna jako niezmiennik}
\input{50-families/hyperbolic_volume}
\input{50-families/hyperbolic_quantum}
