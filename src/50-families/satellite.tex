...
Pytanie, czy satelita ma co najmniej tyle skrzyżowań co jego towarzysz, zadano już w~K2 \cite[problem 1.67]{kirby1997}.
Baker, Motegi \cite{baker2024} nazywają hipotezę o~$km^2$ skrzyżowaniach hipotezą o~indeksie skrzyżowaniowym satelity; przez $m$ rozumieją tam geometryczną liczbę owinięć wzorca, czyli najmniejszą liczbę punktów przecięcia wzorca z~dowolnym dyskiem w~bryle $V$, którego brzegiem jest południk.
\index[persons]{Baker, Kenneth}%
\index[persons]{Motegi, Kimihiko}%
Wiadomo o~niej niewiele \cite[problem 1.2]{baykur2026}.
Lackenby \cite{lackenby2014} pokazał oszacowanie ze stałą $10^{-13}$ zamiast $m^2$ (patrz sekcja~\ref{sec:hyperbolic}).
\index[persons]{Lackenby, Marc}%
Kalfagianni, Lee \cite{kalfagianni2023} udowodnili, że jeśli $K$ jest nietrywialnym węzłem adekwatnym, który ma adekwatny diagram o~spinie równym zero, to dla dubli Whiteheada $\mathrm{Wh}_\pm(K)$ zachodzi $\crossing \mathrm{Wh}_\pm(K) = 4 \crossing K + 2$.
Wzorzec dubla Whiteheada ma geometryczną liczbę owinięć równą $2$, więc wynik jest zgodny z~hipotezą: $4 \crossing K + 2 \ge 2^2 \crossing K$.
\index[persons]{Kalfagianni, Efstratia}%
\index[persons]{Lee, Christine}%
Kalfagianni, McConkey \cite{kalfagianni2024} pokazali, że dla adekwatnego $K$ jego $(p, q)$-kabel $K_{p,q}$ (węzeł okrąża $q$ razy rdzeń torusa) spełnia $\crossing K_{p,q} \ge q^2 \crossing K + 1$.
\index[persons]{McConkey, Rob}%
Baker, Motegi twierdzą, że hipoteza jest prawdziwa, jeśli wzorzec o~geometrycznej liczbie owinięć co najmniej $2$ zawiera dostatecznie dużo pełnych skrętów.
...
Nazywamy go wiernym.
% DICTIONARY;faithful homeomorphism;homeomorfizm wierny;-
...
\index[persons]{Whitten, Wilbur}%
    ...
    Ale my wiemy już z~twierdzenia Gordona-Lueckego, że różne węzły pierwsze mają różne dopełnienia... co jakoś kończy dowód.
\end{proof}
