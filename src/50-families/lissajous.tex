...
\begin{proof}
    ...
    Jeżeli jedna z częstotliwości jest parzysta, to węzeł $K$ jest 2-okresowy (fakt~\ref{prp:lissajous_two_periodic}).
    Niech $J$ będzie jego węzłem ilorazowym.
    Z faktu~\ref{prp:murasugi_periodic} (dla $n=2^1$ oraz $\lambda = 1$) dostajemy kongruencję $\alexander_K(t) \equiv t^k \alexander_J(t)^2 \mod 2$ dla pewnej liczby całkowitej $k$: znak $\pm$ nie ma znaczenia modulo $2$, zaś czynnik $1 + t + \ldots + t^{\lambda-1}$ jest równy $1$.
    Wielomiany $\alexander_K$ oraz $\alexander_J$ są symetryczne w~zmiennych $t$ i~$1/t$, a~$\alexander_J \not\equiv 0 \mod 2$, bo $\alexander_J(1) = \pm 1$.
    Zatem $t^k = t^{-k}$, czyli $k = 0$ i~mamy równość
    \begin{equation}
...
\begin{proof}
    Nietrywialny węzeł o dwóch mostach nie może być silnie dodatnio achiralny (Hartley, Kawauchi \cite{hartley1979}).
    Jeśli jest węzłem Lissajous, jedna z jego częstotliwości okazuje się być parzysta.
    Wtedy jego węzeł ilorazowy jest trywialny i~kongruencja~\ref{eqn:lissajous_squared} daje $\alexander_K(t) \equiv 1 \mod 2$.
\end{proof}
