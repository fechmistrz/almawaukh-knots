...
\index{grupa!węzła}%
    ...
    \begin{equation}
        \pi(K) := \pi_1 \left(\R^3 \setminus K\right),
    \end{equation}
...
\index[persons]{Papakyriakopoulos, Christos}%
...
Sumy proste przeciwnie oraz tak samo skrętnych trójlistników mają różne sygnatury (patrz uwaga za wnioskiem \ref{cor:acheiral_signature}), więc nie są równoważne.
...
\index[persons]{Whitten, Wilbur}%
    ...
    Whitten \cite{whitten1987} pokazał, że węzły o~izomorficznych grupach mają homeomorficzne dopełnienia.
    Wkrótce po tym Gordon, Luecke \cite{gordon1989} udowodnili, że nietrywialna chirurgia Dehna na nietrywialnym węźle nigdy nie daje sfery $S^3$, a~stąd wynika, że każdy homeomorfizm dopełnień węzłów można przedłużyć do homeomorfizmu $S^3$ w~siebie posyłającego jeden węzeł na drugi jako zbiory.
...
Kawauchi \cite[s. 73]{kawauchi1996} i prawie cała reszta świata definiują grupę splotu jako grupę podstawową jego dopełnienia.
